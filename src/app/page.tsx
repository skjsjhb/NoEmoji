...
import { wsRequest } from "@/app/actions/util";
...
export default async function Home() {
    ...
    async function getEmojiHistory(): Promise<{ id: number, date: Date }[]> {
        const emojiResponse = await wsRequest<{ emoji: number; time: number }[]>({
            op: "emoji.query",
            uid: uid
        });
    ...
    async function getGlobalEmojiHistory(): Promise<{ id: number, date: Date, uid: string }[]> {
        ...
        const emojiResponse = await wsRequest<{ emoji: number; time: number; uid: string }[]>({
            op: "emoji.query"
        });
